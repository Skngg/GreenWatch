...
bool LPS22HH_SHub_RegRead(I2CMaster* driver, uint8_t addr, uint8_t* value) {
	...
	bool ready = false;
	while (!ready) {
		if (!LSM6DSO_RegRead(driver, LSM6DSO_SHUB_REG_STATUS_MASTER, &data))
			return false;
		if (data & 0x40)
			ready = true;
	}
	...
	// Read data from LPS22HH saved into SENSOR_HUB_0 reg
...
bool LPS22HH_OpenViaHost(I2CMaster* driver) {
	...
	if (!LSM6DSO_DisableI3C(driver)) {
		// UART_Print(uart_m4_debug,
		//     "ERROR: Disabling I3C Failed for LSM6DSO.\r\n");
		return false;
	}
	...
	// Configure SLAVE0 for onboard LPS22HH
	// Pass-through is impossible due to active INT2 pin (not grounded)
